...
def generate_answer_stream(context: str, question: str):
    """
    Yields chunks of the answer from Ollama using retrieved document context.
    """
    prompt = f"""
You are a precise enterprise knowledge assistant.
...
"""
    ...
    try:
        ...
        yield "AI model timeout. Please try again."
    except requests.exceptions.ConnectionError:
        yield "Ollama server not running. Please start `ollama serve`."
    except Exception as e:
        logger.exception("AI generation stream failed: %s", e)
        yield "AI generation error. Please try again later."
